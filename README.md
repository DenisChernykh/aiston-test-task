...
- Реализован экспорт отфильтрованных заявок в CSV (кнопка «Экспорт»).
...
5. Нажать «Экспорт» и проверить, что скачивается CSV-файл с текущим набором отфильтрованных заявок.
...
### 12. Экспорт отфильтрованных заявок в CSV — ✅
- Файлы:
  - [`src/features/request-actions/model/export-requests-to-csv.ts`](src/features/request-actions/model/export-requests-to-csv.ts)
  - [`src/features/request-actions/ui/request-actions-desktop.tsx`](src/features/request-actions/ui/request-actions-desktop.tsx)
  - [`src/pages/requests/model/use-requests-page-controller.ts`](src/pages/requests/model/use-requests-page-controller.ts)
- Комментарий: экспорт текущей выборки (с учетом фильтров) в CSV и уведомления о результате.
...
7. Нажать `Экспорт` и проверить:
   - если есть строки в текущей выборке, скачивается CSV-файл;
   - если строк нет, показывается уведомление `Нечего экспортировать`.
8. Нажать «Создать новую заявку», заполнить поля, прикрепить файл и отправить форму.
9. После клика «Создать заявку» проверить:
   - модалка закрылась;
   - в консоли браузера появился лог `createRequestDraft`.
10. Проверить мок-состояния данных по URL:
